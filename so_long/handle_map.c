...
/*   Updated: 2024/01/15 17:21:30 by ciusca           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	show_window(t_data *mlx)
{
	...
	map_parsing(mlx);
	if (ft_strlen(mlx->matrix[0]) > 29 || mlx->matrix_i > 15)
	{
		ft_printf("map to big\n");
		exit_function(mlx);
	}
	mlx->win = mlx_new_window(mlx->mlx, 64 * rows, 64 * cols, "so_long");
	display_map(mlx, 8);
}
